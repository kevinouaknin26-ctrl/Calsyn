...
export function renderInviteEmail(p: InviteEmailParams): { subject: string; html: string; text: string } {
  const subject = `Invitation à rejoindre ${p.organisationName} sur Calsyn`
  ...
  <tr><td align="center">
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width:560px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 20px rgba(134,59,255,0.08);">
      ...
      <tr><td align="center" style="padding:24px 40px 0 40px;">
        ...
        <p style="margin:0;font-size:15px;line-height:1.6;color:#475569;text-align:center;"><strong style="color:#0f172a;">${escapeHtml(p.inviterName)}</strong> vous invite à rejoindre <strong style="color:#0f172a;">${escapeHtml(p.organisationName)}</strong> sur Calsyn, le dialer intelligent pour les équipes commerciales.</p>
      </td></tr>
...
${p.inviterName} vous invite à rejoindre ${p.organisationName} sur Calsyn.
...
—
Calsyn · Le dialer intelligent pour les équipes commerciales`
