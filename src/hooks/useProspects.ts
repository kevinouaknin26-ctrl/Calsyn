...
export function useProspects(listId: string | null) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('prospects')
        .select('id, list_id, organisation_id, name, phone, phone2, phone3, phone4, phone5, email, company, title, sector, linkedin_url, website_url, status, crm_status, call_count, last_call_at, snoozed_until, do_not_call, created_at')
        .eq('list_id', listId)
        .order('created_at', { ascending: true })
      if (error) throw error
