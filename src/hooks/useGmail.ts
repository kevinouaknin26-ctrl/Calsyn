...
export function useGmail() {
  ...
  const sendEmail = useCallback(async (input: { to: string; subject: string; body: string; threadId?: string }): Promise<{ ok?: boolean; messageId?: string; threadId?: string; error?: string }> => {
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) return { error: 'No session' }
