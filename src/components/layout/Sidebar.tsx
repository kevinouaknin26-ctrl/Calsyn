    ...
    <>
      ...
      >
        ...
        <nav className={`flex-1 flex flex-col ${expanded ? 'items-stretch px-2' : 'items-center'} gap-0.5`}>
          ...
          <NavItem to="/app/dashboard" label="Dashboard & analyse" icon={<svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>} />
      ...
      {open && (
        <>
          ...
          <div className={`absolute z-50 bg-white rounded-xl border border-gray-200 shadow-xl py-1 min-w-[240px] ${expanded ? 'left-0 bottom-full mb-1' : 'left-full bottom-0 ml-2'}`}>
            ...
            <AccountMenuItem onClick={() => { setOpen(false); navigate(isSuperAdmin ? '/app/super-admin' : '/app/contacts') }} icon="home">
              {isSuperAdmin ? 'Clients Calsyn' : 'Mes contacts'}
            </AccountMenuItem>
            {!isSuperAdmin && (
