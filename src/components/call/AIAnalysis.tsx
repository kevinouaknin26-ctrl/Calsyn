    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-teal-600 font-semibold">Transcription et analyse en cours...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-5">
      ...
      {call.ai_summary && Array.isArray(call.ai_summary) && call.ai_summary.length > 0 && (
        <div className="p-3 rounded-xl bg-teal-50 border border-teal-100">
          <p className="text-[10px] font-bold text-teal-600 uppercase tracking-wider mb-2">Resume</p>
          {call.ai_summary.map((line, i) => (
            <p key={i} className="text-xs text-gray-600 mb-1 flex gap-2"><span className="text-teal-500">—</span>{line}</p>
          ))}
        </div>
      )}
        ...
        <div>
          ...
          <div className="max-h-40 overflow-y-auto text-xs leading-relaxed p-3 rounded-xl bg-gray-50">
            {call.ai_transcript.split('\n').map((line, i) => (
              <p key={i} className={`mb-1 ${line.startsWith('Speaker 0') ? 'text-teal-600' : 'text-gray-500'}`}>{line}</p>
            ))}
          </div>
        </div>
      )}
