      ...
      <div className="fixed inset-0 bg-black/15 flex items-start justify-center pt-[5vh] z-40">
        <div className="bg-white rounded-2xl shadow-xl w-[860px] max-h-[85vh] flex overflow-hidden">
          ...
          <div className="w-[300px] p-5 border-r border-gray-100 flex flex-col overflow-y-auto">
            ...
            {/* LinkedIn + globe icons (Minari exact — sous le nom) */}
            <div className="flex gap-1.5 ml-9 mb-3">
              <div className={`w-5 h-5 rounded flex items-center justify-center text-[9px] font-bold ${prospect.linkedin_url ? 'bg-blue-50 text-blue-500 border border-blue-200' : 'bg-gray-50 text-gray-300 border border-gray-200'}`}>in</div>
              <div className={`w-5 h-5 rounded flex items-center justify-center ${prospect.website_url ? 'bg-gray-50 text-gray-500 border border-gray-200' : 'bg-gray-50 text-gray-300 border border-gray-200'}`}>
            ...
            {/* Call button (Minari exact — teal pill + dropdown + camera + tools) */}
            <div className="flex items-center gap-2 mb-5">
              <button onClick={() => onCall(prospect)} disabled={!providerReady || isInCall}
