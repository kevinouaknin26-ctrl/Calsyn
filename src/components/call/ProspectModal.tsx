    ...
    <div className={`border border-gray-100 rounded-xl mb-2 overflow-hidden transition-all ${open ? 'bg-white' : 'bg-gray-50/50'}`}>
      ...
      {open && (
        <div className="px-4 pb-4 animate-fade-in">
          ...
          {/* Player audio — vrai <audio> avec curseur + bouton télécharger */}
          {call.recording_url && (
            <div className="mb-3 space-y-1.5">
              <audio controls className="w-full h-8 rounded-lg" src={proxyRecordingUrl(call.recording_url!)} preload="metadata" />
              <div className="flex justify-end">
                <a href={proxyRecordingUrl(call.recording_url!)} download={`appel_${call.created_at?.slice(0, 10)}.mp3`} className="text-[11px] text-gray-400 hover:text-gray-600 flex items-center gap-1">
