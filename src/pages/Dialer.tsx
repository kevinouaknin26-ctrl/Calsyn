...
import { useProspectLists, useProspects, useAddProspect, useCreateProspectField, SMART_LIST_LABELS, SMART_LIST_IDS } from '@/hooks/useProspects'
...
function CallSettingsDropdown({ open, onToggle, parallel, setParallel, callLicense, autoRotate, setAutoRotate, voicemail, setVoicemail, completeTask, setCompleteTask, maxAttempts, setMaxAttempts, attemptPeriod, setAttemptPeriod, phoneField, setPhoneField, selectedFromNumber, setSelectedFromNumber, phoneNumbers }: CallSettingsProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-10 w-[440px] bg-white dark:bg-[#ede6f3] rounded-xl shadow-lg border border-gray-200 dark:border-[#d4cade] z-50 p-5 space-y-4 animate-slide-down overflow-y-auto" style={{ maxHeight: '560px' }}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={e => {
                          ...
                          if (existing) {
                            ...
                            // Multi-select (in) dès qu'on a une liste de valeurs distinctes connues → checkboxes pour cocher plusieurs
...
// ── MultiSelectFilter ────────────────────────────────────────────
// Dropdown checkboxes pour filtrer une colonne par plusieurs valeurs
// (Excel/HubSpot style). Stocke la sélection en string CSV "val1,val2"
// pour rester compatible avec le filtre op:'in' existant côté évaluation.
function MultiSelectFilter({ options, value, labelize, onChange }: {
  options: string[]
  value: string
  ...
  const selected = useMemo(() => new Set(value ? value.split(',').map(s => s.trim()).filter(Boolean) : []), [value])
