...
export default function Dialer() {
  ...
  return (
    <div className="min-h-screen bg-[#f0faf4] p-4 pl-2">
      ...
      <div className="px-5 pt-4 pb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {/* Menu "..." */}
            <div className="relative">
              <button onClick={() => setShowListMenu(!showListMenu)} className="text-[13px] text-gray-300 hover:text-gray-500 px-1">...</button>
              {showListMenu && (
                <div className="absolute left-0 top-6 bg-white rounded-xl shadow-lg border border-gray-200 z-50 py-1.5 w-44 animate-slide-down">
                  {(isAdmin || isManager) && (
                    <button onClick={() => {
                    ...
                    }} className="w-full text-left px-3 py-2 text-[12px] text-gray-600 hover:bg-gray-50 flex items-center gap-2">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                      Exporter en CSV
                    </button>
                  )}
                  {!(isAdmin || isManager) && (
                    <p className="px-3 py-2 text-[11px] text-gray-400">Export réservé aux admins</p>
                  )}
                  <button onClick={() => setShowListMenu(false)} className="w-full text-left px-3 py-2 text-[12px] text-gray-600 hover:bg-gray-50 flex items-center gap-2">
      ...
      {(showCallSettings || showFilters || showListMenu) && <div className="fixed inset-0 z-40" onClick={() => { setShowCallSettings(false); setShowFilters(false); setShowListMenu(false) }} />}
    </div>
  )
}
