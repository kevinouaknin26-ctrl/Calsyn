...
// ══════════════════════════════════════════════════════════════════
// SECTION 6b : MESSAGERIE VOCALE PERSO
// ══════════════════════════════════════════════════════════════════
function VoicemailRecorder() {
  ...
  // Charge la messagerie existante (signed URL 1h)
  useEffect(() => {
  ...
  // Sync texte quand profile change
  ...
  const deleteVoicemail = async () => {
    ...
    if (!confirm('Supprimer votre messagerie vocale ?')) return
  ...
  return (
    <div className="space-y-3">
      <div>
        <h3 className="text-[13px] font-bold text-gray-800">Ma messagerie vocale</h3>
        <p className="text-[11px] text-gray-400 mt-0.5">
          Joué quand un appelant tombe sur votre ligne et que vous ne répondez pas.
        </p>
      </div>
      ...
      {/* Preview enregistrement en cours */}
      {recordedUrl && (
        <div className="p-3 rounded-lg bg-violet-50 border border-violet-200 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={uploadRecording} disabled={uploading}
              ...
              {uploading ? 'Upload…' : 'Sauvegarder comme messagerie'}
            </button>
        ...
        <div>
          {recording ? (
            <button onClick={stopRecording}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-red-500 text-white font-semibold text-[13px] hover:bg-red-600 animate-pulse">
              <span className="w-2.5 h-2.5 rounded-full bg-white" />
              Arrêter ({fmtTime(elapsed)})
            </button>
          ) : (
            <button onClick={startRecording}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border-2 border-violet-200 text-violet-600 font-semibold text-[13px] hover:bg-violet-50">
...
export default function Settings() {
  ...
  const visibleSections = SECTIONS.filter(s => {
    ...
    if (perms.isSdr && !['call-settings', 'phone-fields', 'incoming-calls', 'account'].includes(s.id)) return false
    return true
  })
