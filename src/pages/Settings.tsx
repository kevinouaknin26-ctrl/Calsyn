...
function EmailSignatureEditor() {
  ...
  const [imageUrl, setImageUrl] = useState(profile?.email_signature_image_url || '')
  const [saving, setSaving] = useState(false)
  ...
  useEffect(() => {
    ...
    setImageUrl(profile?.email_signature_image_url || '')
  }, [profile?.email_signature, profile?.email_signature_image_url])
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('profiles').update({
      ...
      email_signature_image_url: imageUrl || null,
    }).eq('id', profile.id)
    setSaving(false)
    if (error) { alert(`Erreur : ${error.message}`); return }
    setSaved(true)
  ...
  const uploadImage = async (file: File) => {
    ...
    const { error } = await supabase.storage.from('email-signatures').upload(path, file, { cacheControl: '3600', upsert: false })
    if (error) { alert(`Erreur upload : ${error.message}`); setUploading(false); return }
    const { data: pub } = supabase.storage.from('email-signatures').getPublicUrl(path)
  ...
  const dirty = signature !== (profile?.email_signature || '') || imageUrl !== (profile?.email_signature_image_url || '')
  ...
  return (
    <div>
      ...
      <p className="text-[11px] text-gray-400 mb-3">Appendée automatiquement à chaque email envoyé depuis Calsyn (texte + image).</p>
...
function IntegrationsSection() {
  ...
  // Intégrations PERSONNELLES (chaque user connecte son propre compte) — visibles pour tous
  const personal = [
    {
      ...
      description: 'Synchronise tes RDV (Calendar) et tes emails (Gmail bidirectionnel)',
    },
  ]

  // Intégrations ORG (admin seulement)
  const orgLevel = [
  ...
  const integrations = perms.isAdmin ? [...personal, ...orgLevel] : personal
        ...
        {integrations.map(integ => (
          <div key={integ.name} className="flex items-center gap-3 p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            ...
            {integ.soon ? (
              ...
              <button
                onClick={() => {
                  if (integ.name.startsWith('Google')) gcalConnect()
                }}
...
export default function Settings() {
  ...
  const visibleSections = SECTIONS.filter(s => {
    ...
    // SDR : sections perso (call settings, fiche perso, intégrations personnelles Google)
    if (perms.isSdr && !['call-settings', 'phone-fields', 'incoming-calls', 'account', 'integrations'].includes(s.id)) return false
    return true
  })
